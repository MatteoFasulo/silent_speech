...
class H5EmgDataset(Dataset):
            ...
            if 'voiced' in h5:
                ...
                for sess in h5['voiced']:
                    if sess in voiced_session_paths:
                        dirs.append(EMGDirectory(len(dirs), voiced_session_paths[sess], False, exclude_from_testset=has_silent))
    ...
    def __getitem__(self, i):
        ...
        sample = {
            ...
            'text':            text,
            'text_int':        torch.from_numpy(np.array(
                                  self.text_transform.text_to_int(text),
                                  dtype=np.int64)),
            'text_int_lengths': len(text),
            'session_ids':     torch.full((emg.shape[0],),
                                   fill_value=d.session_index,
                                   dtype=torch.int64),
